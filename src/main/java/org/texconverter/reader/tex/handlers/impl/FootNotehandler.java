...
import org.texconverter.dom.impl.FootNoteImpl;
 ...
 * @author tfrana
 */
public class FootNotehandler implements CommandHandler {

    /**
     * {@inheritDoc}
     */
    public void handle(final Command cmd, final AbstractBuilder builder) {
        if ("footnote".equals(cmd.getName())) {
            builder.consumeNode(build(cmd, builder), false);
        }
    }

    private FootNoteImpl build(final Command cmd,
            final AbstractBuilder builder) {
        FootNoteImpl result = new FootNoteImpl();
        
        String argument = (String)cmd.getMandatoryArgs().get(0);
        
        result.setContent(argument);
